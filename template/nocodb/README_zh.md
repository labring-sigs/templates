...
NocoDB 可以将数据库转换为协作式智能表格和无代码应用。此模板会在 Sealos Cloud 上部署 NocoDB，并创建持久化应用存储。
...
此 Sealos 模板使用固定版本镜像 `nocodb/nocodb:0.265.1` 运行 NocoDB Web 服务。Sealos 会创建 `/usr/app/data` 持久卷、内部服务和带 HTTPS 的公网访问入口。
...
- **一键部署**：通过一个模板部署 NocoDB、持久化存储、服务和公网入口。
...
2. 检查自动生成的应用名称和访问域名，或直接保留默认值。
3. 等待部署完成，通常需要 2-4 分钟。完成后会跳转到 Canvas。
4. 从 App 卡片打开生成的 NocoDB 访问地址，并完成 NocoDB 首次启动设置。
...
- **资源卡片**：点击 StatefulSet、Service 或 Ingress 卡片修改配置。
...
打开生成的访问地址后，按 NocoDB 首次启动流程创建账号。如果已有账号，请使用 NocoDB 内部创建的凭据登录。

### 应用仍在启动

首次冷启动时，NocoDB 初始化本地数据可能需要几分钟。请在 Sealos Canvas 中等待 NocoDB StatefulSet 显示为 `Ready`。

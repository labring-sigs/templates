...
The deployment also provisions a managed PostgreSQL 14.8.0 cluster (Kubeblocks) and a managed Redis 7.0.6 replication topology with Sentinel for caching and queue-related functions. A PostgreSQL init job enables required extensions, and an admin-init job automatically creates the first Quay administrator account during first deployment.
...
- PostgreSQL 14.8.0 (Kubeblocks cluster)
- Redis 7.0.6 with Sentinel (Kubeblocks cluster)

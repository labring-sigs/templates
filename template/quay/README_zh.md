...
部署过程中还会自动创建 PostgreSQL 14.8.0（Kubeblocks）和 Redis 7.0.6（含 Sentinel 的主从拓扑）作为后端依赖。模板内置 PostgreSQL 初始化任务用于启用扩展，同时通过管理员初始化任务在首次部署时创建第一个 Quay 管理员账号。
...
- PostgreSQL 14.8.0（Kubeblocks 集群）
- Redis 7.0.6 + Sentinel（Kubeblocks 集群）
- 镜像数据持久化存储卷
